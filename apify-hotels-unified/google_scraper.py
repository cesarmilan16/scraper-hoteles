...
def scrape_hotel(
    ...
    with sync_playwright() as p:
        ...
        try:
            # Primero navegar a google.com para establecer cookies/sesión
            page.goto("https://www.google.com/?hl=es", wait_until="domcontentloaded", timeout=30000)
            time.sleep(2)
            accept_cookies(page)
            time.sleep(2)
            ...
            seen = set()
            for r in reviews:
                body_lower = (r.body or "").strip().lower()
                if body_lower in OWNER_REPLY_NOISE or not body_lower:
                    continue
                key = (
